...
| Copy-based image exporter | Create-new-only regular output, exact preview/source binding, private partial construction, full source copy, candidate writes, target reinspection, namespace/content manifest equality, source/candidate/manifest/direction-bound schema-v4 escrow sidecar, and before/after source SHA-256 | Implemented and exercised in both directions; verified names are published only after complete candidate and escrow flushes; rich and ten-payload edge candidates pass independent read-only checkers and FUSE mounts; Windows validation and feature-specific corpus remain |
