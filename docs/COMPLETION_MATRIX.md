...
| Activation authorization | The final authorization type is opaque and public forgery compile-fails | Neither serializer currently qualifies; adjacent transaction/verification evidence structures remain publicly constructible and must become opaque before activation can be enabled |
...
| Copy-based image exporter | Create-new-only regular output, exact preview/source binding, uniquely named partial construction, full source copy, candidate writes, target reinspection, namespace/content manifest equality, source/candidate/manifest/direction-bound schema-v4 escrow sidecar, and before/after source SHA-256 | Implemented and exercised in both directions; atomic no-clobber publication currently requires hard-link support in the output directories, and parent-directory durability barriers remain platform work; rich and ten-payload edge candidates pass independent read-only checkers and FUSE mounts; Windows validation and feature-specific corpus remain |
